...
# ====================================================
# outer join
# - 좌/우측 기준 테이블의 모든 행을 relation(조인의 결과)에 포함하는 join
# - Left [outer] join
# - right [outer] join
...
# -> join 결과 : 21행
# 원인? employee.dept_code 에 값이 없는(NULL) 행(하동운, 이오리)이
#    조인 결과(relation)에 포함되지 않음
select
    a.EMP_ID,
    a.EMP_NAME,
    a.DEPT_CODE,
...
# 테스트를 위해 대량의 데이터가 필요할 때 / natural join 실패했을 때 cross join이 나타남 -> 실패 여부 확인
...
select * from tbl_category; # 테이블 안에서 조인 확인

select
    child.category_code,
    child.category_name,
    parent.category_code as "상위 카테고리"
from
    tbl_category child
join
    tbl_category parent
...
join
    department d
on
    e.dept_code = d.dept_id    # location_id
join
    location l
on

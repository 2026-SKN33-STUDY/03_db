...
select *                        #|------------|
from tbl_menu                   #|  메인 쿼리  |
where category_code = (         #|___________|
    SELECT
        category_code
    FROM
        tbl_menu
...
# 1. 4(4, 5, 6,... , 10)번 카테고리 메뉴 중 가장 비싼 메뉴 가격 조회
...
# menu_name, category_code, category_name을 조회하고 싶은데
# tbl_menu에는 category_name이 없음 그럴 때 join 안 쓰고 하는 방법
# 메뉴명, 카테고리코드, 카테고리명 조회
select
    menu_name,
    category_code,
    (select category_name
     ...
     where sub.category_code = main.category_code) category_name # 4 = '한식'
from
    tbl_menu main;
...
# ==============================================
select
    m.menu_name, m.menu_name, c.category_name
from
    tbl_menu m
join
    tbl_category c

...
# - 데이터베이스 '스키마(객체)'를
#   만들고(CREATE), 수정하고(ALTER), 삭제하는(DROP) 구문
# - DDL 구문은 실행 시 바로 DB에 반영된다. (TCL과 달리)
...
create table product(
    ...
    name varchar(100) not null comment '상품명',    # 값 필수 옵션
...
# 테이블의 메타정보(각종 정보)를 조회하는 구문
# menudb의 product 테이블의 정보 확인
select
    *
from
...
### 제약 조건(constraint)
# - 테이블 컬럼에 붙어
#   INSERT, UPDATE 시 각 컬럼에 기록되는 값에 대한 조건을 설정하는 방법1111
...
# Duplicate entry '3-100-2026-06-12 11:31:58' for key 'order_composite_pk.PRIMARY'

# =================================================================
### FOREIGN KEY 외래키
# - 참조(REFERENCE)된 다른 테이블에서 제공하는 값만 사용 가능
# - 참조 무결성을 위배하지 않기 위해 사용
# - 다른 테이블의 PK 또는 UNIQUE가 설정된 컬럼만 가능하다
...
CREATE TABLE IF NOT EXISTS user_foreignkey1 (
    ...
    FOREIGN KEY (grade_code)
		REFERENCES user_grade (grade_code)  # 테이블 컬럼
) ENGINE=INNODB;
...
alter table product
modify name varchar(100) null;  # null 허용

# 컬럼명 변경
alter table product
change name product_name varchar(100) not null;

desc product;   #(name -> product_name)
